// ── Vercel Cron — Daily Scout Background Scan ──────────────────────────────────
// Runs daily at 7am PT. Fetches recent permits for all active users, scores
// them against behavioral profiles, upserts to scout_opportunities, and
// sends WhatsApp/SMS notifications for hot leads.
//
// Required SQL migration (run once in Supabase):
//   ALTER TABLE behavioral_profiles ADD COLUMN IF NOT EXISTS notification_phone TEXT;
//   ALTER TABLE behavioral_profiles ADD COLUMN IF NOT EXISTS notification_email TEXT;

import { NextRequest, NextResponse } from "next/server";
import { createClient } from "@supabase/supabase-js";
import type { ScoutOpportunity, ScoreBreakdown } from "@/lib/types";

// ── Auth ──────────────────────────────────────────────────────────────────────

function authorized(req: NextRequest): boolean {
  const secret = process.env.CRON_SECRET;
  if (!secret) return process.env.NODE_ENV !== "production"; // dev only — blocked in prod
  const auth = req.headers.get("authorization") ?? "";
  return auth === `Bearer ${secret}`;
}

// ── Supabase (service role) ───────────────────────────────────────────────────

function getDB() {
  const url = process.env.SUPABASE_URL!;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY!;
  return createClient(url, key);
}

// ── Scoring ───────────────────────────────────────────────────────────────────

interface PermitRow {
  id?: string;
  address: string;
  city: string;
  state?: string;
  project_type: string;
  value?: number;
  builder_company?: string;
  builder_phone?: string;
  builder_email?: string;
  applicant_company?: string;
  owner_company?: string;
  issued_date?: string;
  status?: string;
  description?: string;
  additional_info?: string;
}

interface BehavioralProfile {
  user_id: string;
  top_project_types: string[];
  top_cities: string[];
  value_range_min?: number;
  value_range_max?: number;
  win_rate_by_type: Record<string, number>;
  top_companies: string[];
  total_wins: number;
  notification_phone?: string;
  notification_email?: string;
  // From store: user may have trades/cities in setup (we use behavioral profile cities)
}

function scorePermit(permit: PermitRow, profile: BehavioralProfile): ScoreBreakdown {
  // 1. Request fit — project type match (0-30)
  let request_fit = 0;
  const ptLower = (permit.project_type ?? "").toLowerCase();
  const descLower = (permit.description ?? "").toLowerCase();
  if (profile.top_project_types?.length) {
    for (const type of profile.top_project_types) {
      if (ptLower.includes(type.toLowerCase()) || descLower.includes(type.toLowerCase())) {
        // Win rate boost
        const wr = profile.win_rate_by_type?.[type] ?? 0;
        request_fit = Math.min(30, 20 + Math.round(wr * 10));
        break;
      }
    }
    if (!request_fit) request_fit = 8; // no match but profile exists
  } else {
    request_fit = 15; // neutral — no profile data yet
  }

  // 2. Timing — recency of permit (0-20)
  let timing = 0;
  if (permit.issued_date) {
    const daysAgo = Math.floor((Date.now() - new Date(permit.issued_date).getTime()) / 86400000);
    if (daysAgo <= 3) timing = 20;
    else if (daysAgo <= 7) timing = 17;
    else if (daysAgo <= 14) timing = 13;
    else if (daysAgo <= 30) timing = 8;
    else timing = 3;
  } else {
    timing = 5;
  }

  // 3. Commercial — value range fit (0-15)
  let commercial = 0;
  const v = Number(permit.value ?? 0);
  if (v > 0 && profile.value_range_min && profile.value_range_max) {
    if (v >= profile.value_range_min && v <= profile.value_range_max) commercial = 15;
    else if (v >= profile.value_range_min * 0.5 && v <= profile.value_range_max * 2) commercial = 8;
    else commercial = 3;
  } else if (v > 50_000) {
    commercial = 8; // unknown range but non-trivial project
  } else {
    commercial = 5;
  }

  // 4. Relationship — placeholder (0-25)
  // Real relationship scoring runs in Scout chat with full contact lookup.
  // Cron uses a simplified heuristic: known company in top_companies gets a boost.
  let relationship = 0;
  const companyName = (permit.builder_company ?? permit.applicant_company ?? "").toLowerCase();
  if (profile.top_companies?.length && companyName) {
    for (const c of profile.top_companies) {
      if (companyName.includes(c.toLowerCase()) || c.toLowerCase().includes(companyName)) {
        relationship = 20;
        break;
      }
    }
  }
  if (!relationship) relationship = 5; // unknown — no warm path detected

  // 5. Data quality (0-10)
  let confidence = 0;
  if (permit.builder_company) confidence += 3;
  if (permit.builder_phone || permit.builder_email) confidence += 4;
  if (permit.value) confidence += 3;

  const total = request_fit + timing + commercial + relationship + confidence;
  const priority: ScoreBreakdown["priority"] = total >= 70 ? "hot" : total >= 50 ? "warm" : "watch";
  return { total, request_fit, timing, commercial, relationship, confidence, priority };
}

function permitToOpportunity(permit: PermitRow, score: ScoreBreakdown, userId: string): ScoutOpportunity {
  const priority = score.total >= 70 ? "hot" : score.total >= 50 ? "warm" : "watch";
  const id = `cron-${userId.split("@")[0]}-${(permit.address ?? "unknown").replace(/\W+/g, "-").slice(0, 30)}-${permit.issued_date?.slice(0, 10) ?? "nodate"}`;

  const matchReasons = [];
  if (score.request_fit >= 20) matchReasons.push({ label: "Strong type match", detail: `Matches your ${permit.project_type} experience`, type: "trade" as const });
  if (score.timing >= 17) matchReasons.push({ label: "Just issued", detail: "Permit filed in the last 3 days", type: "timing" as const });
  if (score.commercial >= 12) matchReasons.push({ label: "In your value range", detail: `$${(Number(permit.value) / 1000).toFixed(0)}K project`, type: "value" as const });
  if (score.relationship >= 18) matchReasons.push({ label: "Known company", detail: "This builder is in your contact history", type: "relationship" as const });

  const daysAgo = permit.issued_date
    ? Math.floor((Date.now() - new Date(permit.issued_date).getTime()) / 86400000)
    : null;
  const timing = daysAgo !== null ? (daysAgo === 0 ? "just filed" : daysAgo === 1 ? "yesterday" : `${daysAgo} days ago`) : "recently";

  return {
    id,
    score: score.total,
    priority,
    timing,
    matchReasons,
    suggestedAction: score.relationship >= 18
      ? "Reach out — you know this builder"
      : score.timing >= 17
      ? "Contact now while it's fresh"
      : "Research and reach out this week",
    actionType: permit.builder_phone ? "call" : permit.builder_email ? "email" : "research",
    scoreBreakdown: score,
    primarySource: "permit",
    sourceRecords: [],
    project: {
      id,
      name: permit.project_type ?? "Construction Project",
      address: permit.address ?? "",
      city: permit.city ?? "",
      type: permit.project_type ?? "",
      value: Number(permit.value ?? 0),
      stage: "permitted",
      issuedDate: permit.issued_date ?? "",
      description: permit.description ?? "",
    },
    company: {
      id: `co-${(permit.builder_company ?? "unknown").replace(/\W+/g, "-").slice(0, 20)}`,
      name: permit.builder_company ?? permit.applicant_company ?? "Unknown Builder",
      type: "gc",
      recentProjects: 0,
    },
    relationship: {
      hasWarmPath: score.relationship >= 18,
      strength: score.relationship >= 18 ? "direct" : "none",
      summary: score.relationship >= 18 ? "Known company" : "No warm path detected",
      path: [],
      confidence: score.relationship,
    },
  } as ScoutOpportunity;
}

// ── Main handler ──────────────────────────────────────────────────────────────

export const maxDuration = 60; // Vercel Pro max for cron functions

export async function GET(req: NextRequest) {
  if (!authorized(req)) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const db = getDB();
  const batchId = `cron-${new Date().toISOString().slice(0, 10)}`;
  const results: Record<string, { hot: number; warm: number; watch: number }> = {};

  // ── 1. Get all active users with behavioral profiles ──────────────────────
  const { data: profiles, error: profileErr } = await db
    .from("behavioral_profiles")
    .select("user_id, top_project_types, top_cities, value_range_min, value_range_max, win_rate_by_type, top_companies, total_wins, notification_phone, notification_email")
    .gt("updated_at", new Date(Date.now() - 30 * 86400000).toISOString()) // active in last 30 days
    .limit(30);

  if (profileErr || !profiles?.length) {
    return NextResponse.json({ ok: true, message: "No active profiles found", batchId });
  }

  const sevenDaysAgo = new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10);

  // ── 2. Process each user ──────────────────────────────────────────────────
  for (const profile of profiles) {
    const userId = profile.user_id;
    const cities = (profile.top_cities as string[]) ?? [];
    if (!cities.length) continue; // no cities configured — skip

    // Query recent permits for this user's cities
    const cityOr = cities.slice(0, 5).map((c: string) => `city.ilike.${c}`).join(",");
    const { data: permits } = await db
      .from("permits")
      .select("address, city, state, project_type, value, builder_company, builder_phone, builder_email, applicant_company, owner_company, issued_date, status, description")
      .or(cityOr)
      .gte("issued_date", sevenDaysAgo)
      .order("issued_date", { ascending: false })
      .limit(50);

    if (!permits?.length) continue;

    // Score and filter
    const scored = permits
      .map((p) => ({ permit: p as PermitRow, score: scorePermit(p as PermitRow, profile as BehavioralProfile) }))
      .filter(({ score }) => score.total >= 40) // watch or above
      .sort((a, b) => b.score.total - a.score.total)
      .slice(0, 10); // top 10 per user per day

    if (!scored.length) continue;

    // Upsert to scout_opportunities
    let hot = 0, warm = 0, watch = 0;
    const upsertRows = [];

    for (const { permit, score } of scored) {
      const opp = permitToOpportunity(permit, score, userId);
      const priority = opp.priority;
      if (priority === "hot") hot++;
      else if (priority === "warm") warm++;
      else watch++;

      upsertRows.push({
        id: opp.id,
        user_id: userId,
        source: "cron_scan",
        score: score.total,
        priority,
        project_data: opp,
        dismissed: false,
        liked: false,
        contacted: false,
        pipeline_stage: "new",
        updated_at: new Date().toISOString(),
      });
    }

    // Batch upsert (ignore if already exists — don't overwrite user actions)
    if (upsertRows.length) {
      await db.from("scout_opportunities").upsert(upsertRows, {
        onConflict: "id",
        ignoreDuplicates: true, // don't overwrite dismissed/liked/contacted status
      });
    }

    results[userId] = { hot, warm, watch };

    // ── 3. Notify via SMS/WhatsApp if hot leads found ───────────────────────
    const phone = (profile as BehavioralProfile).notification_phone;
    if (hot > 0 && phone) {
      try {
        const appUrl = process.env.NEXTAUTH_URL ?? "https://scout.buildmapper.ca";
        const msg = `Scout found ${hot} hot lead${hot > 1 ? "s" : ""} in ${cities.slice(0, 2).join(" & ")} today. Open BuildMapper to review → ${appUrl}/opportunities`;
        await fetch(`${appUrl}/api/notify/whatsapp`, {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-internal": "1" },
          body: JSON.stringify({ phone, message: msg }),
        });
      } catch { /* notification failure is non-blocking */ }
    }
  }

  const totalHot = Object.values(results).reduce((s, r) => s + r.hot, 0);
  const totalWarm = Object.values(results).reduce((s, r) => s + r.warm, 0);
  const usersNotified = Object.keys(results).length;

  return NextResponse.json({
    ok: true,
    batchId,
    usersScanned: usersNotified,
    summary: { hot: totalHot, warm: totalWarm },
    perUser: results,
  });
}
